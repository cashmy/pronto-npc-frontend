// src/components/NpcSystemRaceLookup/NpcSystemRaceLookup.stories.tsx
...
import NpcSystemRaceLookup from "./NpcSystemRaceLookup"; // Adjust path as necessary
import { NpcSystemRaceSelectRecord } from "../../dataModels/npc_system_races"; // Adjust path

const meta: Meta<typeof NpcSystemRaceLookup> = {
  title: "Components/NpcSystemRaceLookup",
  component: NpcSystemRaceLookup,
  tags: ["autodocs"],
  argTypes: {
    value: {
      control: "text",
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "NpcSystemRaceLookup component that fetches options from a live backend. Ensure your backend is running at `http://localhost:8000` for these stories to fetch data.",
      },
    },
  },
};
...
type Story = StoryObj<typeof NpcSystemRaceLookup>;
...
const InteractiveLookup: React.FC<Partial<Story["args"]>> = (storyArgs) => {
  ...
  const handleChange = (
    textVal: string,
    selectedOpt?: NpcSystemRaceSelectRecord
  ) => {
    setValue(textVal);
...
export const FetchErrorObservation: Story = {
  args: {
    label: "Races (Observe Fetch Error)",
    npcSystemId: 2,
    value: "",
    fullWidth: true,
    margin: "normal",
...
export const CustomLookupIcon: Story = {
  args: {
    label: "NPC Race - diff Icon (Live Data)",
    npcSystemId: 2,
    value: "",
    fullWidth: true,
    margin: "normal",
...
export const WithExternalErrorProp: Story = {
  args: {
    label: "NPC Race (External Validation)",
    npcSystemId: 2,
