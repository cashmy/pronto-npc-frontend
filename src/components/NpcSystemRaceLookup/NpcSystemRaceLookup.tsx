...
// src/components/NpcSystemRaceLookup/NpcSystemRaceLookup.tsx
...
import NpcSystemRaceService from "../../services/npc_system_races.service"; // Adjust path
import { NpcSystemRaceSelectRecord } from "../../dataModels/npc_system_races"; // Adjust path

// Define the props for our NpcSystemRaceLookup component
export type NpcSystemRaceLookupProps = Omit<
  TextFieldProps,
...
> & {
  ...
  onChange: (
    textValue: string,
    selectedOption?: NpcSystemRaceSelectRecord
  ) => void;
...
const NpcSystemRaceLookup: React.FC<NpcSystemRaceLookupProps> = ({
  value,
  onChange,
  ...
  const baseId = useId();
  const menuId = "npc-system-race-lookup-menu" + baseId; // Unique ID for the menu
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      setFetchError(null);
      try {
        const response = await NpcSystemRaceService.getRecordsForSelect(
          systemIdAsNumber
        );
  ...
  const handleMenuItemClick = (option: NpcSystemRaceSelectRecord) => {
...
export default NpcSystemRaceLookup;
