...
// src/components/NpcSystemProfessionLookup/NpcSystemProfessionLookup.tsx
...
import NpcSystemProfessionService from "../../services/npc_system_profession.service"; // Adjust path
import { NpcSystemProfessionSelectRecord } from "../../dataModels/npc_system_professions"; // Adjust path

// Define the props for our NpcSystemProfessionLookup component
export type NpcSystemProfessionLookupProps = Omit<
  TextFieldProps,
...
> & {
  ...
  onChange: (
    textValue: string,
    selectedOption?: NpcSystemProfessionSelectRecord
  ) => void;
...
const NpcSystemProfessionLookup: React.FC<NpcSystemProfessionLookupProps> = ({
  value,
  onChange,
  ...
  const baseId = useId();
  const menuId = "npc-system-profession-lookup-menu" + baseId; // Unique ID for the menu
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      ...
      setFetchError(null);
      try {
        const response = await NpcSystemProfessionService.getRecordsForSelect(
          systemIdAsNumber
        );
  ...
  const handleMenuItemClick = (option: NpcSystemProfessionSelectRecord) => {
    ...
    <>
      ...
      <Menu
        ...
        slotProps={{
          ...
          paper: {
            style: {
              // Attempt to match the text field's width if anchorEl is the TextField
              width: anchorEl?.getBoundingClientRect().width,
              // minWidth: 240, // Minimum width for the menu
              maxHeight: 240,
            },
          },
        }}
        ...
        //   },
        // }}
        // Ensure the menu is anchored to the TextField or a relevant element
        // If anchorEl is set to the TextField, Popper can use it.
        anchorOrigin={{
          vertical: "bottom",
          horizontal: "left",
        }}
...
export default NpcSystemProfessionLookup;
